...
module test_order_book_side_ask;
    
    // parameters
    localparam N = 8;
    localparam DESCENDING = 0;
    ...
    initial begin
        $dumpfile("order_book_side_ask_tb.vcd"); // output waveform file
        $dumpvars(0, test_order_book_side_ask);    // 0 = dump all levels of hierarchy, starting from this module
    end
    ...
    initial begin
        ...
        do_insert(16'h0032, 16'h0008, 16'h000C, 16'h000B); // price 50 — LOWER than 100, lands BEFORE it on ask side

        if (valid !== 8'b00000011) begin
            $display("FAIL: valid mask = %b, expected 8'b00000011 after second reduce + new insert", valid);
        end else if (price[0*16 +: 16] !== 16'h0032 || orderID[0*16 +: 16] !== 16'h000C) begin
            $display("FAIL: new lower-priced order did not land at slot 0. price=%h orderID=%h",
                    price[0*16 +: 16], orderID[0*16 +: 16]);
        end else if (quantity[1*16 +: 16] !== 16'h000A || orderID[1*16 +: 16] !== 16'h000B) begin
            $display("FAIL: original reduced order did not shift correctly to slot 1. quantity=%h orderID=%h",
                    quantity[1*16 +: 16], orderID[1*16 +: 16]);
        end else begin
            $display("PASS: reduced quantity persisted correctly through an unrelated insert, and shifted correctly on the ask side");
        end
        print_book;
